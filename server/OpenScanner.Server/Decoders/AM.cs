...
public class AM : DSDBase
{
    ...
    public override string GetCommandLine(Channel channel)
    {
        ...
        return $"stdbuf -o0 rtl_fm -f {channel.Frequency}M -M am -s 8k -r {outputRate} -g 35 -p 0 -l 0 - | /usr/bin/ffmpeg -f s16le -ar {outputRate} -ac 1 -i - -af 'highpass=f=300,lowpass=f=4000,volume=4.0' -f s16le -ar {outputRate} -ac 1 -fflags nobuffer -flags low_delay -flush_packets 1 - -loglevel quiet";
    }
