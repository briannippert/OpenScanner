...
public class NFM : DSDBase
{
    ...
    public override string GetCommandLine(Channel channel)
    {
        ...
        int captureRate = 12000;

        // Simplified pipeline: No tee/dsd-fme for now to guarantee audio flow.
        // rtl_fm outputs s16le at 12k rate. 
        // ffmpeg resamples to 48k and applies bandpass/volume.
        // -l 0: No squelch once started (the scanner already detected a carrier).
        return $"stdbuf -o0 rtl_fm -f {channel.Frequency}M -M fm -s {captureRate} -g 35 -p 0 -l 0 - | stdbuf -o0 /usr/bin/ffmpeg -f s16le -ar {captureRate} -ac 1 -i - -af 'highpass=f=300,lowpass=f=4000,volume=4.0' -f s16le -ar {outputRate} -ac 1 -fflags nobuffer -flags low_delay -flush_packets 1 - -loglevel quiet";
    }
